...
function EchoNest(apiKey) {
	...
	this.staticPl = new StaticPl(this);
...
function StaticPl(en) {
	this.en = en;
}

StaticPl.prototype.create = function(config, callback, error) {
	this.en.apiRequest('playlist/static', config,
		function(data) {
			console.log(data);
			callback(data);
		},
...
Catalog.prototype.list = function(api_key, callback, error) {
    this.en.apiRequest('catalog/list', {api_key : api_key}, callback, error);
}
...
Catalog.prototype.update = function(catalogID, updateBlock, callback, error) {
...
Catalog.prototype.read = function(id, start, results, bucket, callback, error) {
    args = {id:id, results: results, start:start, results: results};
    if (bucket !== "") {
        _.extend(args, {bucket: bucket});
    }

    this.en.apiRequest('catalog/read', args, callback, error);
};
